...
import { reopenEditComment, resolveEditComment } from "@/features/capabilities/asset_review_with_timecodes/api";
import type { EditComment } from "@/features/capabilities/asset_review_with_timecodes/types";
...
export function EditCommentsList({ comments, onUpdate, onJumpTo }: Props) {
    ...
    <ul className="divide-y">
      {sorted.map((c) => (
        <li key={c.id} className="flex items-start gap-3 px-2 py-2 text-sm">
          ...
          <span className={`flex-1 ${c.resolved_at ? "text-muted-foreground line-through" : ""}`}>
            {c.body}
          </span>
