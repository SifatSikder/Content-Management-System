...
DirectorRoles = require_role(Role.CEO, Role.ASSISTANT_DIRECTOR, Role.JUNIOR_DIRECTOR)
UnlockerRoles = require_role(Role.CEO, Role.ASSISTANT_DIRECTOR)
...
@projects_router.post(
    "/lock",
    summary="Lock the script (director+ only)",
)
async def post_lock(
    project: Annotated[ProjectModel, Depends(require_project_access(ProjectAccess.VIEW))],
    user: Annotated[CurrentUser, Depends(DirectorRoles)],
    session: SessionDep,
) -> dict[str, str]:
    try:
...
@projects_router.post(
    "/unlock",
    summary="Unlock the script (CEO + Assistant Director only)",
)
async def post_unlock(
    project: Annotated[ProjectModel, Depends(require_project_access(ProjectAccess.VIEW))],
    user: Annotated[CurrentUser, Depends(UnlockerRoles)],
    session: SessionDep,
) -> dict[str, str]:
    try:
