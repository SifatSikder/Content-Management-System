...
import {
  type ScriptEnhancementCandidate,
  listScriptEnhancementCandidates,
  requestScriptEnhancement,
...
export function ScriptRequestFeedbackDialog({
  project,
  open,
  onOpenChange,
  onRequested,
}: Props) {
  const [candidates, setCandidates] = useState<ScriptEnhancementCandidate[] | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await listScriptEnhancementCandidates(project.id);
        if (cancelled) return;
        setCandidates(res.items);
        // Default checked: everyone EXCEPT reviewers whose latest
        // signoff on this script is already `looks_good`.
        setSelected(
          new Set(
            res.items
  ...
  async function onConfirm() {
    ...
    setSubmitting(true);
    try {
      const res = await requestScriptEnhancement(
        project.id,
        Array.from(selected),
      );
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Pick the people who should review the current script. Each one
