...
import { listScriptVersionSignoffs } from "@/features/script_versioning/api";
import type {
  ScriptSignoff,
  ScriptVersion,
...
export function ScriptVersionHistoryItem({ projectId, version }: Props) {
  const [expanded, setExpanded] = useState(false);
  const [signoffs, setSignoffs] = useState<ScriptSignoff[] | null>(null);
  ...
  async function toggle() {
    ...
    if (next && signoffs === null && !loading) {
      ...
      setError(null);
      try {
        const rows = await listScriptVersionSignoffs(projectId, version.id);
        setSignoffs(rows);
      } catch {
